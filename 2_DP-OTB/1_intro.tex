...
Let $\calR$ be a distribution on $(\calX,\|\cdot\|)$ and $\vecr\sim\calR$. $\calR$ is said to be an \emph{$(\alpha,\RDPdist)$-RDP distribution} if (i) $\E[\vecr]=0$, (ii) $\E[\|\vecr\|^2]=\RDPdist^2$, and (iii) for all $\sigma, \rho>0$ and $\vecmu,\vecmu'\in\calX$ such that $\sigma\ge \|\vecmu-\vecmu'\|/\rho$, then $D_\alpha(\sigma \vecr+\vecmu \| \sigma \vecr+\vecmu')\le \alpha\rho^2/2$.
\label{def:DP-OTB:RDP-dist}
\end{definition}

\begin{theorem}
If $\calM:\Xi^T\to(\calX,\|\cdot\|)$ has senstivity $s$ and $\calR$ is a $(\alpha,\RDPdist)$-RDP distribution on $\calX$, then $\tilde\calM = \calM + \vecr$ is $(\alpha, \alpha\rho^2/2)$-RDP where $\vecr\sim \sigma\calR$ and $\sigma = s/\rho$.
\label{thm:DP-OTB:RDP-mechanism}
\end{theorem}

The Guassian distribution $\calN(0,\matI)$ is an $(\alpha,\sqrt{d})$-RDP distribution on the Euclidean space. 
In general, $(\alpha, \RDPdist)$-RDP distribution extends Gaussian mechanism to Banach space beyond the Euclidean sapce, and $\RDPdist$ captures the additional noise factor to achieve privacy which depends on both the distribution and the geometry of $\calX$. Since the goal of this thesis does not focus on privacy mechanism design, we treat this as a general definition and does not provide mechanisms on non-Eucliean spaces. Notably, this definition decouples privacy mechanism from the subsequent convergence analysis.
...
\label{sec:DP-OTB:tree-mechanism}
...
The following results formally state the privacy guarantee of the tree mechanism. The proofs can be found in Appendix \ref{app:prelim:tree-mechanism:privacy}.

\begin{restatable}{theorem}{ThmTreePrivacy}
\label{thm:tree-mechanism:privacy}
...
\begin{equation}
    \rho \le \sqrt{2\log_2(2n)} \cdot \max_{k\in[n]} \frac{\max_{i\le k} s_i}{\sigma_k}.
    \label{eq:tree-mechanism:privacy}
\end{equation}
\end{restatable}

\begin{restatable}{theorem}{ThmTreeNoise}
\label{thm:tree-mechanism:noise}
If $\calR$ is an $(\alpha,\RDPdist)$-RDP distribution and $\|\cdot\|_\ast^2$ is $\lambda$-strongly convex with respect to $\|\cdot\|_\ast$, then
\begin{equation}
    \E[\|\tree(k)\|^2] \le \frac{2\log_2(2k)}{\lambda} \RDPdist^2 \max_{i\le k} \sigma_i^2.
    \label{eq:tree-mechanism:noise} 
\end{equation}
\end{restatable}