...
If $\calM_k:\calZ_{i_k}\mapsto \vecx_k$ is $(\alpha, \rho_k)$-RDP, then the composition $\calM:\calZ_T \mapsto (\vecx_1,\ldots,\vecx_k)$ is $(\alpha, \rho_{1:n})$-RDP where $\rho_{1:n}=\sum_{k=1}^n \rho_k$.
...
At the core of our proposed conversion algorithms, it incorporates a variant of the tree mechanism, which is an advanced privacy mechanism to aggregate the streaming sum of a sequence of outputs. The formal setup of this problem is as follows. Consider a Banach space $(\calX,\|\cdot\|)$ and a dataset $\calZ_T=(\xi_1,\ldots,\xi_T)\in\Xi^T$, which is partitioned into $n$ parts $\calZ_T = \calZ_{i_1} \sqcup \ldots \sqcup \calZ_{i_n}$. There is a sequence of algorithms $\calM_1,\ldots,\calM_n$ such that $\calM_1:\calZ_{i_1}\mapsto \vecx_1\in\calX$ and $\calM_{k+1}:\vecx_{1:k}\times \calZ_{i_k} \mapsto \vecx_{k+1}\in\calX$ iteratively. The \emph{streaming sum} of these algorithms is $\calS_k: \calZ_T \mapsto \vecx_{1:k}$, and their composition is the algorithm $\calS: \Xi^T\to \calX^n$ such that $\calZ_T\mapsto (\vecx_1,\ldots,\vecx_{1:n})$.
...
\begin{equation*}
    \rho \le \RDPdist\sqrt{2\ln n} \cdot \max_{k\in[n]} \frac{\max_{i\le k} s_i}{\sigma_k}.
\end{equation*}
\end{theorem}
