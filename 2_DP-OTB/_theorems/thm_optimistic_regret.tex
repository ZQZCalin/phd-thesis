\begin{theorem}
\label{thm:optimistic}
% Suppose $\|\cdot\|^2$ is $\lambda$-strongly convex, $W$ is bounded by $D$, and $\ell$ is $G$-Lipschitz and $H$-smooth. 
Suppose Assumption \ref{as:sc-norm} - \ref{as:smooth} hold, and $\calD$ is a $(V,\alpha)$-RDP distribution. Set $\weight_t=t^k$ and $\sigma_t^2$ as defined in \eqref{eq:RDP-noise},
If the online learner is optimistic (satisfying \eqref{eq:optimistic-OL}) with $t$-th gradient $\bar{g}_t=g_t+\gamma_t$ and $t$-th hint $\hat{g}_t=\bar{g}_{t-1}$, then
\begin{align*}
