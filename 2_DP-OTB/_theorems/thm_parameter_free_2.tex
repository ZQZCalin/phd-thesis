\begin{theorem}
\label{thm:parameter-free-main}
Following the assumptions and notations in Theorem \ref{thm:parameter-free-2-norm-step-1}, if we replace $\ell_t(w)$ in Algorithm \ref{alg:DP-OTB} with regularized loss $\bar \ell_t(w) = \ell_t(w) + \xi_t\|w\|_2 + \nu_t\|w\|_2^2$ and denote the associated regret as $\overline \regret_t$, then with probability at least $1-\delta$, $\L(x_T)-\L(x^*)$ is bounded by:
\begin{align*}
    % \L(x_T)-\L(x^*) 
    % &\leq 
