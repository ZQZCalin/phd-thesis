...
\begin{lemma}\label{lem:DP-OTB:renyigaussian}
...
\begin{proof}[Proof of Lemma \ref{lem:DP-OTB:renyigaussian}]
...
In the main text, we focus on Renyi differential privacy, and we defined RDP-distribution (Definition \ref{def:DP-OTB:RDP-dist}) accordingly. We can always extend our result in a pure differential privacy setting.

\newcommand{\DPnorm}{\text{$(K,\calD)$-DP-able}}
\begin{definition}[$V$-DP distribution]
\label{def:DP-OTB:DP-dist}
...
The tree aggregation described in Appendix \ref{app:DP-OTB:tree-aggregation-RDP} also works for pure DP mechanisms as well. Therefore, if we assume $\calD$ in Algorithm \ref{alg:DP-OTB:main} with a $V$-DP distribution and change the definition of $\sigma_t^2$ in \eqref{eq:DP-OTB:RDP-noise} correspondingly, Algorithm \ref{alg:DP-OTB:main} can be modified to an purely $\epsilon$-DP mechanism.

Next, we can show that exponential mechanism in general norm satisfies this definition:

\begin{lemma}
\label{thm:DP-OTB:exponential-mechanism-general-norm}
