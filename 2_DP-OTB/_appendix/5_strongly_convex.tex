\section{Proofs for the Strongly Convex Case (Section \ref{sec:DP-OTB_strongly-convex})}
\label{app:DP-OTB:strongly-convex}

\begin{lemma}
\label{lem:DP-OTB:basic-lemma-sc}
...
\begin{align}
    ...
    \label{eq:DP-OTB:sc-1}
\end{align}
With the same argument in the proof of Lemma \ref{lem:DP-OTB:basic-lemma}, we can show:
\begin{align*}
...
\end{align*}
Consequently, moving $\sum_{t=1}^T \weight_tf(x_t)$ to the right side and taking the telescopic sum in \eqref{eq:DP-OTB:sc-1} gives:
\begin{align*}
    -\weight_{1:T}f(x^*)
...
This lemma immediately implies Lemma \ref{lem:DP-OTB:strongly-convex-regularized-loss}.
...
\end{align*}
Upon substituting this equation into Lemma \ref{lem:DP-OTB:basic-lemma-sc}, we get:
\begin{align*}
...
\begin{proposition}
\label{prop:DP-OTB:subdiff-chain-rule}
