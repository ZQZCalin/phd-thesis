\section{Proofs for the Parameter-free Case (Section \ref{sec:DP-OTB_parameter-free})}
\label{app:DP-OTB:parameter-free}
...
\label{lem:DP-OTB:nSG-concentration}
...
Recall that $\weight_t\nabla f(x_t)-g_t=\sum_{i=1}^t X_i$ (Lemma \ref{lem:DP-OTB:variance-expectation-bound}), where
\[
...
\begin{lemma}
\label{lem:DP-OTB:X-R-nSG}
Suppose Assumption \ref{as:DP-OTB:bounded-domain} - \ref{as:DP-OTB:smooth} hold w.r.t. the 2-norm, and suppose $\calD$ is a $(V,\alpha)$-RDP distribution and is $\sigma_\calD$-sub-Gaussian, i.e.,
\begin{align*}
...
\begin{proof}
Since we assume $\E[\nabla\ell(x,z)]=\nabla f(x)$ for all $x$, $\E[X_i|X_{1:i-1}]=0$. Also, since $\calD$ is a $(V,\alpha)$-RDP distribution (Definition \ref{def:DP-OTB:RDP-dist}) and $R_i=\sigma_i\tilde R_i$'s are independent, $\E[R_i|R_{1:i-1}]=\E[\tilde R_i]=0$. 

For the second part, in Lemma \ref{lem:DP-OTB:variance-expectation-bound} we proved that
\begin{align*}
...
Hence, by Lemma \ref{lem:DP-OTB:normSG}, $X_i|X_{1:i-1}$ and $R_i|R_{1:i-1}$ are norm-sub-Gaussian.
\end{proof}
...
\input{2_DP-OTB/_theorems/thm_parameter_free}

\begin{proof}
We start with Eq. \eqref{eq:DP-OTB:parameter-free-base}:
\begin{align}
    ...
    &\leq R_T(x^*) + \sum_{t=1}^T \left(\|\weight_t\nabla f(x_t)-g_t\|_2+\|\gamma_t\|_2\right) (\|w_t-x^*\|_2). \label{eq:DP-OTB:parameter-free-base}
\end{align}

\textbf{Step 1.}
By Lemma \ref{lem:DP-OTB:nSG-concentration} and \ref{lem:DP-OTB:X-R-nSG}, for each $t$, with probability $1-\delta/2T$,
\begin{align*}
    \|\weight_t\nabla f(x_t)-g_t\|_2 
...
\begin{align}
    ...
    \label{eq:DP-OTB:variance-concentration}
\end{align}
...
\begin{align}
    ...
    \leq & 8C^2\Phi \sum_{t=1}^T \left(\sqrt{\sum_{i=1}^t 2G^2i^4} + \sqrt{\sum_{i=1}^t 2H^2\|w_i-x_{i-1}\|_2^2i^4}\right) \|w_t-x^*\|_2 \label{eq:DP-OTB:step-1}
\end{align}
...
\begin{align}
    ...
    \leq & AH\sum_{t=1}^T \left( \frac{1}{\lambda_t}\sum_{i=1}^t i^4\|w_i\|_2^2 + \frac{1}{\lambda_t}\sum_{i=2}^t i^4\sum_{j=1}^{i-1} \frac{\weight_j\|w_j\|_2^2}{\weight_{1:i-1}} + \lambda_t(\|w_t\|_2^2+\|x^*\|_2^2) \right). \label{eq:DP-OTB:step-1.2}
\end{align}
...
% \end{align*}
\textbf{1.2.2.} For the second term, by Proposition \ref{prop:DP-OTB:alphas}, $\weight_{1:i-1}\geq (i-1)^4/4$, so
\begin{align*}
...
\begin{align*}
    ...
    \leq \frac{3}{t^{1/2}}.
\end{align*}
% We then bound the inner summation with the same argument using the integral inequality. By Proposition \ref{prop:DP-OTB:alphas}, $\sum_{j=t}^i\frac{(j+1)^{2(k-1)}}{\weight_{1:j}}\leq \sum_{j=t}^i\frac{4(j+1)^4}{j^4} \leq 64i$. Hence,
% \begin{align*}
...
% \end{align*}
In conclusion, upon substituting \textbf{1.2.1.} and \textbf{1.2.2.} into \eqref{eq:DP-OTB:step-1.2} and setting $c=14$, we get:
\begin{align*}
    &8C^2\sum_{t=1}^T\sqrt{\sum_{i=1}^t2H^2\|w_i-x_{i-1}\|_2^2i^4}\|w_t-x^*\|_2 \\
...
Moreover, upon substituting \textbf{1.1.} and \textbf{1.2.} into \eqref{eq:DP-OTB:step-1}, we get: with probability at least $1-\delta/2$,
\begin{align*}
    &\sum_{t=1}^T\|\weight_t\nabla f(x_t)-g_t\|_2\|w_t-x^*\|_2 \\
...
\textbf{Step 2:} We can bound $\sum_{t=1}^T\|\gamma_t\|_2\|w_t-x^*\|_2$ in a similar way. By Lemma \ref{lem:DP-OTB:X-R-nSG} and definition of $\sigma_t$ in \eqref{eq:DP-OTB:RDP-noise}, $R_i|R_{1:i-1}$ is mean-zero $\nSG(\sigma_{R_i})$, where
\begin{align*}
    \sigma_{R_i} 
...
\end{align*}
Recall that $A'=8\sqrt{d}\sigma_\calD C^2$. By Lemma \ref{lem:DP-OTB:nSG-concentration}, for each $t$, with probability at least $1-\delta/2T$,
\begin{align}
    \|\gamma_t\|_2 
    ...
    \label{eq:DP-OTB:privacy-concentration}
\end{align}
By union bound, with probability at least $1-\delta/2$,
\begin{align*}
...
In conclusion, we take the union bound on the results from \textbf{step 1.} and \textbf{step 2.} and substitute it back to the starting point \eqref{eq:DP-OTB:parameter-free-base}.  Then with probability at least $1-\delta$,
\begin{align*}
    \weight_{1:T}(f(x_T)-f(x^*))
