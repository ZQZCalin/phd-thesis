...
\eqref{eq:DP-OTB:basic-decomposition-2} decomposes the suboptimality gap $f(\vecx_T)-f(\vecx_\ast)$ into three components: (i) regret $\regret_T(\vecx_\ast)$, (ii) error associated with variance of $\vecg_t$, measured by $\|\weight_t\nabla f(\vecx_t)-\vecg_t\|_*$, and (iii) noise from the tree mechanism, measured by $\|\overline\vecr_t\|_*$. While the first regret term depends on the OCO subroutine, the other two terms could be bounded as follows.

To get a tight bound, we observe that $\weight_t\nabla f(\vecx_t)-\vecg_t$ and $\overline\vecr_t$ are sums of conditionally mean-zero random vectors. With a martingale bound in high dimension with general norm (formally stated as Lemma \ref{lem:DP-OTB:martingale-bound-general-norm}), we can derive the following bounds. 
...
\begin{restatable}[DP-OTB Privacy Noise Bound]{lemma}{LemDpOtbNoiseBound}
...
Suppose $F$ is $G$-Lipschitz, $H$-smooth, and the domain $\calX$ has bounded diameter $D$. Let $\weight_t=t^k$ and define the noise constant $\sigma_t$ in Algorithm \ref{alg:DP-OTB:main} as 
\begin{align}
    \sigma_t
