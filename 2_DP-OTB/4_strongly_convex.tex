...
\label{sec:DP-OTB_strongly-convex}
...
If $ f$ is strongly convex, then we can prove a similar result to Theorem \ref{lem:DP-OTB:basic-lemma} (the proof is in Appendix \ref{app:DP-OTB:strongly-convex}).

\begin{restatable}{lemma}{LemDpotbSC}
\label{lem:DP-OTB:strongly-convex-regularized-loss}
Suppose $f$ is $\mu$-strongly convex w.r.t. $\|\cdot\|$. If we replace $\ell_t(w)=\langle g_t+\overline\vecr_t,w\rangle$ with $\bar \ell_t(w)=\ell_t(w)+\frac{\weight_t\mu}{4}\|w-x_t\|^2$ in Algorithm \ref{alg:DP-OTB:main}, and denote the associated regret by $\overline \regret_T$, then
\begin{align*}
    \weight_{1:T} ( f(x_T) -  f(x^*))
    % &\leq \overline\regret _T(x^*) +\sum_{t=1}^T \langle \weight_t\nabla f(x_t)-g_t-\overline\vecr_t, w_t-x^*\rangle - \frac{\weight_t\mu}{8}\|w_t-x^*\|^2 \\
    &\leq \overline\regret_T(x^*) + \sum_{t=1}^T \frac{2\|\weight_t\nabla f(x_t)-g_t-\overline\vecr_t\|_*^2}{\weight_t \mu}.
\end{align*}
\end{restatable}

Compared to Lemma \ref{lem:DP-OTB:basic-lemma} and Equation \ref{eq:DP-OTB:basic-decomposition-2}, in the strongly convex case, there is an additional term $-\frac{\weight_t\mu}{8}\|w_t-x^*\|^2$, which allows the improved convergence rate:

\begin{theorem}
\label{thm:DP-OTB:strongly-convex}
Suppose Assumption \ref{as:DP-OTB:sc-norm} - \ref{as:DP-OTB:sigma-H} hold, and $\calD$ is a $(\alpha,\RDPdist)$-RDP distribution. Also suppose $ f$ is $\mu$-strongly convex. Set $\weight_t=t^k$ and $\sigma_t^2$ as defined in \eqref{eq:DP-OTB:RDP-noise}. Then $\E[ f(x_T) -  f(x^*)]$ is bounded by:
\begin{align*}
    % \E[ f(x_T)- f(x^*)] &\leq 
...
\end{align*}
Moreover, for all $\bar g_t\in \partial \bar \ell_t(w_t)$, we have $\E[\|\bar g_t\|_*^2] \leq$
\begin{align*}
    t^{2k}\left( 4G^2 + \mu^2D^2 + \frac{16(k+1)^2}{\lambda}\left(\frac{(\sigma_G+D\sigma_H)^2}{t} + \frac{2V(G+DH)^2\log_2^2(2T)}{\rho^2t^2}\right) \right).
\end{align*}
\end{theorem}
...
\end{align*}
where $g_t\in \partial\ell_t(w_t)$. In our case, $\|\cdot\|_2^2$ is $2$-strongly convex and the regularized loss $\bar \ell_t$ is $\frac{\weight_t\mu}{2}$-strongly convex, i.e. $\mu_t=\mu t^k/2$ and $\sum_{i=1}^t \mu_i = O(\mu t^{k+1})$. Therefore, by Theorem \ref{thm:DP-OTB:strongly-convex},
\begin{align*}
...
\begin{proof}[Proof of Theorem \ref{thm:DP-OTB:strongly-convex}]
We have already bounded $\E[\|\weight_t\nabla f(x_t)-g_t\|_*^2]$ and $\E[\|\overline\vecr_t\|_*^2]$ in \eqref{eq:DP-OTB:variance-norm} and \eqref{eq:DP-OTB:main:total-tree-noise} respectively, so
\begin{align*}
    &\E[\|\weight_t\nabla f(x_t)-g_t-\overline\vecr_t\|_*^2] \\
    &\leq 2\E[\|\weight_t\nabla f(x_t)-g_t\|_*^2+\|\overline\vecr_t\|_*^2] \\
...
\end{align*}
Upon substituting this into Lemma \ref{lem:DP-OTB:strongly-convex-regularized-loss} and replace $\weight_t=t^k$, we get:
\begin{align*}
...
For the second part, recall that $\bar \ell_t(w)=\langle g_t+\overline\vecr_t,w\rangle + \frac{\weight_t\mu}{4}\|w-x_t\|^2$. Therefore, for all $\bar g_t\in\partial \bar\ell_t(w)$, $\bar g_t = g_t+\overline\vecr_t + \frac{\beta_t\mu}{4}v$, where $v\in \partial\|w_t-x_t\|^2$.
% Since the linear part is differentiable,
% \begin{align*}
%     \partial\bar{\ell}_t(w) = g_t+\overline\vecr_t + \frac{\weight_t\mu}{4}\partial\|w-x_t\|^2. 
% \end{align*}
...
\begin{align*}
    \E[\|\bar g_t\|_*^2]
    &\leq 4\E[\|\weight_t\nabla f(x_t)\|_*^2 + \|\weight_t\nabla f(x_t)-g_t\|_*^2 + \|\overline\vecr_t\|_*^2 + \|\tfrac{\weight_t\mu}{4} v\|_*^2]
\end{align*}
Here $v\in\partial\|w_t-x_t\|^2$. We bound the first term by Lipschitz, the second by \eqref{eq:DP-OTB:variance-norm}, and the third by \eqref{eq:DP-OTB:main:total-tree-noise}. Moreover, by chain rule (Proposition \ref{prop:DP-OTB:subdiff-chain-rule}), we can show that $\|v\|_* \leq 2D$, so:
\begin{align*}
    &\E[\|\bar g_t\|_*^2] \leq 4t^{2k}G^2 + \frac{16(k+1)^2(\sigma_G+D\sigma_H)^2t^{2k-1}}{\lambda} \\
