...
\input{2_DP-OTB/_theorems/lem_strongly_convex_regret}

% First, recall that $x_t$ are determined by $\{g_1+\gamma_1,\ldots,g_{t-1}+\gamma_{t-1}\}$, and we proved in Theorem \ref{thm:DP-OTB:main-privacy}
...
Suppose Assumption \ref{as:DP-OTB:sc-norm} - \ref{as:DP-OTB:sigma-H} hold, and $\calD$ is a $(V,\alpha)$-RDP distribution. Also suppose $ f$ is $\mu$-strongly convex. Set $\weight_t=t^k$ and $\sigma_t^2$ as defined in \eqref{eq:DP-OTB:RDP-noise}. Then $\E[ f(x_T) -  f(x^*)]$ is bounded by:
\begin{align*}
    % \E[ f(x_T)- f(x^*)] &\leq 
...
We have already bounded $\E[\|\weight_t\nabla f(x_t)-g_t\|_*^2]$ and $\E[\|\gamma_t\|_*^2]$ in \eqref{eq:DP-OTB:variance-norm} and \eqref{eq:DP-OTB:gamma-bound} respectively, so
\begin{align*}
...
\begin{align*}
    ...
    \intertext{Here $v\in\partial\|w_t-x_t\|^2$. We bound the first term by Lipschitz, the second by \eqref{eq:DP-OTB:variance-norm}, and the third by \eqref{eq:DP-OTB:gamma-bound}. Moreover, by chain rule (Proposition \ref{prop:DP-OTB:subdiff-chain-rule}), we can show that $\|v\|_* \leq 2D$, so:}
