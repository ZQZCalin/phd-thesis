...
We also make use of the ``tree mechanism'' \cite{dwork2010differential, chan2011private}, which is a technique that allows for private release of \emph{running sums} of potentially sensitive data (Algorithm~\ref{alg:DP-O2NC:tree}).
...
Randomized smoothing is a well known technique that converts a possibly non-smooth function to a smooth approximation \citep{flaxman2005online, duchi2012randomized, lin2022gradient}.
...
\begin{lemma}
\label{lem:DP-O2NC:uniform-smoothing}
...
% We defer the proof to Appendix \ref{app:DP-O2NC:uniform-smoothing}. 
As a corollary, finding an $(\delta,\epsilon)$-stationary point of $\hat F_\delta$ is sufficient to find an $(2\delta,\epsilon)$-stationary point of $F$. The formal statement is as follows (\citet[Lemma 4]{kornowski2023algorithm}; proof is presented in Appendix \ref{app:DP-O2NC:pre} for completeness). 

\begin{corollary}
\label{cor:DP-O2NC:smooth-reduction}
...
This paper is organized in the following order. In Section \ref{sec:DP-O2NC_estimator}, we introduce the zeroth-order estimators. In Section \ref{sec:DP-O2NC_algorithm}, we present the O2NC algorithm combined with the private variance-reduced gradient oracle. In Section \ref{sec:DP-O2NC_privacy}, we demonstrate the improved privacy guarantee provided by the tree mechanism. We conclude with the convergence analysis in Section \ref{sec:DP-O2NC_convergence}.