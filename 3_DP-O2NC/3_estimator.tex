...
\label{ch3:sec:estimator}

Let $F(\vecx)=\E_z[f(\vecx,z)]$ and denote $\hat F_\delta$ and $\hat f_\delta$ as the uniform smoothing of $F$ and $f$ respectively.
Using the randomized smoothing technique from Lemma \ref{lem:uniform-smoothing}, we can construct unbiased zeroth-order estimators for both
% $\nabla\hat f(\vecx,z)$ and $\nabla\hat f(\vecx,z) - \nabla\hat f(\vecy,z)$. 
$\nabla\hat F_\delta(\vecx)$ and $\nabla\hat F_\delta(\vecx) - \nabla\hat F_\delta(\vecy)$.
The key properties of these estimators $\grad$ and $\diff$, as defined in Algorithm \ref{alg:first-grad} and \ref{alg:zeroth-diff}, are summarized in the following results.

\begin{lemma}
\label{lem:grad-estimator}
...
A more detailed exploration of this idea is presented in the subsequent sections, especially in Corollary \ref{cor:privacy} and Lemma \ref{lem:oracle-variance}.
