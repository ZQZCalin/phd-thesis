...
\label{sec:Exp-O2NC_alg}

\begin{algorithm}[t]
\caption{Exponentiated O2NC}
\label{alg:Exp-O2NC:O2NC-exp-avg}
\begin{algorithmic}[1]
...
One notable feature of Algorithm \ref{alg:Exp-O2NC:O2NC-exp-avg} is that the update $\Delta_n$ is scaled by an exponential random variable $s_n$. 
Formally, we have the following result:

\begin{restatable}{lemma}{ExponentialScaling}
\label{lem:Exp-O2NC:exp-scaling}
...
In Algorihtm~\ref{alg:Exp-O2NC:O2NC-exp-avg}, we set $s_n\sim\mathrm{Exp}(1)$ and then define $\vecx_n = \vecx_{n-1} + s_n\Delta_n$. Thus, Lemma \ref{lem:Exp-O2NC:exp-scaling} implies that 
...
% In O2NC (refer to Algorithm \ref{alg:Exp-O2NC:o2nc-original} for notations), \citet{cutkosky2023optimal} draw a connection between the expected gradient and the shifting regret by carefully constructing the comparator. Specifically, by setting $\vecu_n=\vecu^k$ for all $n\in I_k=[(k-1)T+1,kT]$ where
% \begin{equation}
...
\label{lem:Exp-O2NC:variance-regularizer}
For any $\beta\in(0,1)$, 
\begin{align*}
...
Furthermore, we'd like to highlight that Lemma \ref{lem:Exp-O2NC:variance-regularizer} provides a strictly better bound on the variance of $\vecy_n$ compared to the possible maximum deviation $\max\|\vecy_n-\overline\vecx_n\|$. For illustration, assume $\Delta_t$'s are orthonormal, then $\max\|\vecy_N-\overline\vecx_N\| \approx \|\vecx_1-\vecx_N\| = O(N)$. On the other hand, Lemma \ref{lem:Exp-O2NC:variance-regularizer} implies that for $n\sim\Unif([N])$, $\E_n[\Var(\vecy_n)] = O(\frac{1}{(1-\beta)^2})$. In particular, we will show that $1-\beta= N^{-1/2}$ when the objective is smooth. Consequently, $\E\|\vecy_n-\overline\vecx_n\| = O(\sqrt{N})$, which is strictly tighter than the deterministic bound of $\max\|\vecy_N-\overline\vecx_N\| = O(N)$.
...
\label{sec:Exp-O2NC_alg-conv}

Now we present the main convergence theorem of Algorithm \ref{alg:Exp-O2NC:O2NC-exp-avg}. This is a very general theorem, and we will prove the convergence bound of a more specific algorithm (Theorem \ref{thm:Exp-O2NC:sgdm}) based on this result. A more formally stated version of this theorem and its proof can be found in Appendix \ref{app:Exp-O2NC:alg}.  

\begin{theorem}
\label{thm:Exp-O2NC:o2nc}
Follow Assumption \ref{as:Exp-O2NC:optimization}. Let $\regret_n(\vecu_n)$ denote the regret w.r.t. $\ell_t(\Delta)=\langle\beta^{-t}\vecg_t,\Delta\rangle + \calR_t(\Delta)$ for $t=1,\ldots,n$ and comparator $\vecu_n$ defined in \eqref{eq:Exp-O2NC:eo2nc-u}. Define $\calR_t(\Delta) = \frac{\mu_t}{2}\|\Delta\|^2$, $\mu_t = \frac{24cD}{\alpha^2}\beta^{-t}$, and $\alpha=1-\beta$, then
\begin{align*}
