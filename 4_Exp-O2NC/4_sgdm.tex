...
\end{equation*}
Let $\mu_t=\beta^{-t}\mu$, $\eta_t=\beta^t\eta$, $\beta=1-\alpha$, $\mu=\frac{24F^*c}{(G+\sigma)\alpha^{5/2}N}$, $\eta=\frac{2F^*}{(G+\sigma)^2N}$, and \\
$\alpha=\max\{N^{-2/3}, \frac{(F^*)^{4/7}c^{2/7}}{(G+\sigma)^{6/7}N^{4/7}}\}$. Then for $N$ large enough such that $\alpha \le \frac{1}{2}$,
\begin{align*}
