...
\begin{assumption}
\label{as:Exp-O2NC:optimization}
...
\input{4_Exp-O2NC/2a_prelim_nonsmooth}

\input{4_Exp-O2NC/2b_prelim_o2nc}


\subsection{Paper Organization}

In Section \ref{sec:Exp-O2NC_alg}, we present the general online-to-non-convex framework, \alg. We  first explain the intuitions behind the algorithm design, and then we provide the convergence analysis in Section \ref{sec:Exp-O2NC_alg-conv}. In Section \ref{sec:Exp-O2NC_sgdm}, we provide an explicit instantiation of our framework, and see that the resulting algorithm is essentially the standard SGDM.
% In section \ref{sec:Exp-O2NC_adam}, we further apply adaptive learning rate.
In Section \ref{sec:Exp-O2NC_lower}, we present a lower bound for finding $(c,\epsilon)$-stationary point.
In Section \ref{sec:Exp-O2NC_experiment}, we present empirical evaluations.